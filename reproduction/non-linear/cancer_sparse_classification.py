...
from models import (MLP, SparseFeatureLinearRegression, SparseFeatureNet, SparseFeatureNetv2)
...
parser.add_argument("--model_name", type=str, default='sparse_feature_net_v2', choices=model_names)
...
def _logistic_regression(X_train, y_train, X_test, y_test, **kwargs):
    clf = LogisticRegression(penalty='l1', solver='saga', C=100, max_iter=100)
...
def _some_net(NetClass, X_train, y_train, X_test, y_test, alpha, device, **kwargs):
    ...
    acc, metric_list = run_classification(
        alpha, X_train, y_train, X_test, y_test, net, device=device, **kwargs)
    return {'acc': acc}
...
def evaluate_model(X_train, y_train, X_test, y_test, model_name, **kwargs):
    if model_name.lower() == 'logistic_regression':
        ...
        metric = _sparse_feature_net(
            SparseFeatureLinearRegression, X_train, y_train, X_test, y_test, **kwargs)
        ...
        metric = _sparse_feature_net(
            SparseFeatureNet, X_train, y_train, X_test, y_test, **kwargs)
    elif model_name.lower() == 'sparse_feature_net_v2':
        """run a joint trained feature selection and end to end network"""
        metric = _sparse_feature_net(
            SparseFeatureNetv2, X_train, y_train, X_test, y_test, **kwargs)
    else:
        raise NotImplementedError
    return metric


def multi_trials_model(X, y, model_name, num_trials=10, **kwargs):
    acc_list = []
    for i in range(num_trials):
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, shuffle=True)
        t0 = time.time()
        metric = evaluate_model(X_train, y_train, X_test, y_test, model_name, **kwargs)
        dt = time.time() - t0
        metric['dt'] = dt
...
if __name__ == "__main__":
    ...
    log_path = "log/" + args.model_name + ".log"
    ...
    for f in os.listdir(r'data'):
        if f.endswith('soft.gz'):
            filepath = os.path.join(r'data', f)
